...
__all__ = ['unet_autoencoder_learner']


def unet_autoencoder_learner(data:DataBunch, arch:Callable, pretrained:bool=True, blur_final:bool=True,
    ...
    model = to_device(DynamicUnetAutoencoder(body, n_classes=data.c, img_size=size, blur=blur, blur_final=blur_final,
...
class UnetBlockAutoencoder(Module):
    ...
    def forward(self, up_in:Tensor) -> Tensor:
        ...
        s = s * 0.0  # -> disable long skip connections for autoencoder
        cat_x = self.relu(torch.cat([up_out, s], dim=1))
        return self.conv2(self.conv1(cat_x))


class DynamicUnetAutoencoder(SequentialEx):
    ...
    def __init__(self, encoder:nn.Module, n_classes:int, img_size:Tuple[int,int]=(256,256), blur:bool=False, blur_final=True, self_attention:bool=False,
        ...
        for i,idx in enumerate(sfs_idxs):
            ...
            unet_block = UnetBlockAutoencoder(up_in_c, x_in_c, self.sfs[i], final_div=not_final, blur=do_blur, self_attention=sa,
                                   **kwargs).eval()
