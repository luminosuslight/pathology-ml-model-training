		...
		Item {
			...
			TextInput {
				...
				Timer {
					...
					interval: 300
					onTriggered: {
                        var searchResult = controller.blockManager().blockList().getSearchResult(searchField.displayText, controller.developerMode)
	...
	Flickable {
		...
		StretchColumn {
			...
			function showAllBlocks() {
                var blockModel = JSON.parse(controller.blockManager().blockList().getJsonBlockModel(controller.developerMode))
...
//				}
			...
			function addBlock(info, level, collapsed) {
				...
				if (info.category.length > level) {
                    ...
                    var entryComponent = Qt.createComponent("qrc:/ui/app/BlockListEntry.qml");
			...
			function getCategory(info, level, collapsed) {
                ...
                var categoryComponent = Qt.createComponent("qrc:/ui/app/BlockListCategory.qml");
