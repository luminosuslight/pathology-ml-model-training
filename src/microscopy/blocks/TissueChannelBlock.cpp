...
bool TissueChannelBlock::s_registered = BlockList::getInstance().addBlock(TissueChannelBlock::info());

TissueChannelBlock::TissueChannelBlock(CoreController* controller, QString uid)
...
{
    connect(&m_filePath, &StringAttribute::valueChanged, this, &TissueChannelBlock::onFilePathChanged);
}

float TissueChannelBlock::pixelValue(int x, int y) const {
...
void TissueChannelBlock::onFilePathChanged() {
